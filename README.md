...
├── tests
│   ├── __init__.py
│   ├── test_close_notice.py
│   ├── test_detector.py
│   ├── test_identity.py
│   ├── test_member_writer.py
│   ├── test_members.py
│   ├── test_monitor_identity.py
│   ├── test_monitor_ordering.py
│   ├── test_monitor_pricing.py
│   ├── test_no_sale.py
│   ├── test_parser.py
│   ├── test_product_consistency.py
│   ├── test_run_monitor_settings.py
│   ├── test_winner.py
│   └── test_workbook.py
...
- `auction/member_writer.py`: 방송 중 추가 회원을 회원명단 엑셀에 저장
...
- `monitor_entry.py`: Windows 설치파일에 포함할 모니터/회원 저장 실행파일 진입점
...
- `tests/test_member_writer.py`: 방송 중 추가 회원 엑셀 저장 테스트
- `tests/test_members.py`: 회원명단 엑셀 로드 테스트
- `tests/test_monitor_identity.py`: 모니터링 표시 아이디 테스트
- `tests/test_monitor_ordering.py`: YouTube 메시지 시간순 처리와 삭제/철회 이벤트 테스트
- `tests/test_monitor_pricing.py`: 경매 입찰가 단위 변환 테스트
- `tests/test_close_notice.py`: 낙찰선/유찰선 주변 숫자 댓글 오인식 방지 테스트
...
- `tests/test_run_monitor_settings.py`: Electron 모니터 실행 설정 보정 테스트
